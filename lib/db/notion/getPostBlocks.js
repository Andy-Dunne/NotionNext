...
// 需要签名/续签的Notion文件类block（含 image；检测②中会单独排除 image，因为其URL已由 mapImgUrl 还原为稳定 attachment: 标识）
const FILE_BLOCK_TYPES = ['pdf', 'audio', 'image', 'video', 'file', 'page']
...
export function hasExpiredSignedUrls(recordMap, bufferMs = 10 * 60 * 1000) {
  ...
  // ① notion-client / refreshSignedUrls 写入 recordMap.signed_urls 的签名URL
  if (Object.values(recordMap?.signed_urls || {}).some(url => isExpiring(url))) {
    return true
  }

  // ② 【新增】新API(app.notion.com)下，file.notion.com / file.notion.so 签名直链可能
  //    直接内嵌在 block 的 properties.source / format.page_cover / file.url 中，并不存在于
  //    recordMap.signed_urls；若不检测，这些链接过期后永远不会触发续签，图片/文件将长时间裂图。
  //    （image 块除外：其URL会被 mapImgUrl 还原为 attachment: 稳定标识，不受签名过期影响）
  return Object.values(recordMap?.block || {}).some(entry => {
    const block = getBlockValue(entry)
    if (!block || !FILE_BLOCK_TYPES.includes(block.type) || block.type === 'image') {
      return false
    }
    const source =
      block.type === 'page'
...
async function refreshSignedUrls(recordMap, cacheKey) {
  ...
  try {
    ...
    files.forEach((file, index) => {
      ...
      // 【新增】若该block自身的source就是 file.notion.* 签名直链，则同步写回新签名URL，
      // 避免每次渲染都因旧签名过期而重复触发续签。
...
function getNotionFileSource(source) {
  ...
  // 【新增】新CDN签名直链 → 还原为 attachment: 标识再请求续签
  // getSignedFileUrls 对 attachment: 格式的解析最稳定，且不受旧签名是否过期影响
  if (/^https?:\/\/file\.notion\.(?:com|so)\/f\/f\//.test(source)) {
    try {
      const parts = new URL(source).pathname.split('/') // ['', 'f', 'f', spaceId, attachmentId, filename...]
      ...
      console.warn('[getNotionFileSource] parse file.notion URL failed:', source, e)
    }
  }
