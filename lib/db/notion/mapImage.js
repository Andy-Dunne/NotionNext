...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // 【新增】Notion 新CDN直链(file.notion.com / file.notion.so) → 还原为 attachment: 标识。
  // 新API(app.notion.com)返回的图片签名直链格式为:
  //   https://file.notion.com/f/f/{spaceId}/{attachmentId}/{filename}?table=block&id=...&expirationTimestamp=...&signature=...
  // 其中 {attachmentId}/{filename} 与旧格式 attachment:{attachmentId}:{filename} 一一对应。
  // 还原后可复用下方 attachment 分支生成稳定的反代URL(与旧API行为一致)，且不受签名1小时过期影响。
  if (/^https?:\/\/file\.notion\.(?:com|so)\/f\/f\//.test(ret)) {
    try {
      const pathParts = new URL(ret).pathname.split('/') // ['', 'f', 'f', spaceId, attachmentId, filename...]
      const attachmentId = pathParts[4]
      const fileName = pathParts.slice(5).join('/')
      if (attachmentId && fileName) {
        ret = `attachment:${attachmentId}:${decodeURIComponent(fileName)}`
      }
    } catch (e) {
      console.warn('[mapImgUrl] parse file.notion URL failed:', ret, e)
    }
  }

  const hasConverted =
     ret.indexOf('https://www.notion.so/image') === 0 ||
     // 【新增】已走反代的 /image/ 链接不再二次转换
     ret.indexOf(BLOG.NOTION_HOST + '/image/') === 0 ||
